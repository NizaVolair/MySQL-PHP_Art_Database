...
<html>
	<head>
		<meta charset="UTF-8">
		<title>Art Archive: Series - By Series</title>
		<link rel="stylesheet" href="style.css" type="text/css">
	</head>

	<body>
		<h1>Artwork by Series</h1>
		...
		<div>
			<table>
				...
				<?php
					// Prepare series selection statement with relevant condition
					if(!($stmt = $mysqli->prepare("SELECT a.title, a.image, s.title, a.width, a.height, a.year FROM artwork a 
													...
													WHERE s.id = ?
													ORDER BY s.title ASC, a.year DESC"))){
						echo "Prepare failed: " . $stmt->errno . " " . $stmt->error;
					}

					// Bind statement to form parameters
					if(!($stmt->bind_param("i", $_POST["Series"]))){
